...
# Peer data is fetched ONLY when the user clicks the button (lazy load).
# Each peer is fetched once and reused for both the table and chart.
# ──────────────────────────────────────────────────────────────────────────────
...
with tab5:
    ...
    # Peers list (no data fetch yet)
    peer_symbols = []
    if current_sector and current_sector in SECTOR_MAPPING:
        peer_symbols = [s for s in SECTOR_MAPPING[current_sector] if s != symbol][:3]

    all_symbols = [symbol] + peer_symbols

    st.markdown("#### 📋 Peer Comparison")
    if peer_symbols:
        st.caption(f"Will compare: {', '.join(all_symbols)}")
    else:
        st.caption("No peers mapped — will show standalone stats for this stock.")

    # ── Cached helper so repeated tab switches don't re-fetch ────────────
    @st.cache_data(ttl=300)
    def _fetch_peer(sym):
        """Fetch 1Y data for one symbol. Cached for 5 minutes."""
        import time
        try:
            time.sleep(0.5)   # small delay to avoid rate limiting
            d = yf.download(f"{sym}.NS", period="1y", progress=False)
            if d.empty or len(d) < 30:
                d = yf.download(f"{sym}.BO", period="1y", progress=False)
            if d.empty or len(d) < 30:
                return None
            c = d['Close']
    ...
    # ── Button-gated fetch — does NOT run on every page load ─────────────
    if st.button("📊 Load Sector Data", type="primary", key="load_sector"):
        st.session_state["sector_loaded"] = True
        st.session_state["sector_symbol"] = symbol   # invalidate if stock changes

    # Reset if user switched stock
    if st.session_state.get("sector_symbol") != symbol:
        st.session_state["sector_loaded"] = False

    if not st.session_state.get("sector_loaded", False):
        st.info("👆 Click **Load Sector Data** to compare this stock with its peers.")
    else:
        # Fetch each peer ONCE, store close series
        peer_closes = {}
        prog = st.progress(0, text="Fetching peer data…")
        for idx, sym in enumerate(all_symbols):
            close_series = _fetch_peer(sym)
            if close_series is not None:
                peer_closes[sym] = close_series
            prog.progress((idx + 1) / len(all_symbols), text=f"Fetched {sym}")
        prog.empty()

        if not peer_closes:
            st.warning("Could not load any peer data. Yahoo Finance may be rate-limiting. Try again in a minute.")
        else:
            # ── Build comparison table from already-fetched data ──────────
            peer_rows = {}
            ...
            if peer_rows:
                ...
                # ── Normalised 1Y chart (reuses peer_closes — no extra fetch) ─
