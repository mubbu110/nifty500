...
def fetch_stock_news(symbol, company_name):
    """
    Fetch stock-specific news from multiple sources with fallbacks.
    ...
    # Multiple fallback feeds - different sources and approaches
    feeds_to_try = [
        ...
        # Financial Times RSS
        "https://markets.ft.com/data/indices/tearsheet/summary?s=NIFTY",
        
        # Alternative: Google News (sometimes works)
        f"https://news.google.com/rss/search?q={symbol}&hl=en-IN&gl=IN&ceid=IN:en",
    ]
        ...
        try:
            ...
            for entry in feed.entries[:25]:
                try:
                    ...
                    # Check if stock-specific (but be more lenient)
                    if not is_stock_specific(title, symbol, company_name):
                        # Try a simpler check - just symbol or company keywords
                        if symbol.lower() not in title.lower() and company_name.lower() not in title.lower():
                            continue
                    ...
                    if isinstance(source, dict):
                        source_name = source.get("title", "News")
                    ...
                    news.append({
                        "title": title,
                        "source": str(source_name)[:30],  # Limit source name length
                        "date": pub_date,
                    })
            ...
            # If we got news from this feed, return it
            if news:
                break
        
        except requests.exceptions.Timeout:
            # Feed took too long, try next one
            continue
        except requests.exceptions.ConnectionError:
            # Network issue, try next one
            continue
        except Exception:
            # Any other error, try next feed
            continue
    
    # Sort by recency (newest first) - handle missing dates gracefully
    try:
        sorted_news = sorted(
            news, 
    ...
    return sorted_news[:MAX_NEWS_ARTICLES]
